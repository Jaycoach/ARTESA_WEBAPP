...
const DashboardBranchLayout = () => {
    ...
    // ✅ OBTENER TODOS LOS DATOS DEL CONTEXTO
    const { isAuthenticated, authType, isLoading, branch, user } = useAuth();
    ...
    useEffect(() => {
        ...
        if (authType !== AUTH_TYPES.BRANCH) {
            console.log('❌ Tipo de usuario incorrecto para dashboard-branch:', authType);
            // Redirigir al dashboard apropiado según el tipo
            const correctDashboard = authType === AUTH_TYPES.USER ? '/dashboard' : '/login';
            navigate(correctDashboard);
            return;
        }

        if (!branch) {
            console.log('⚠️ Datos de sucursal no disponibles');
            // Intentar restaurar desde localStorage como fallback
            const branchData = localStorage.getItem('branchData');
            if (!branchData) {
                console.log('❌ No hay datos de sucursal en localStorage, redirigiendo a login');
                navigate('/login');
                return;
            }
        }

        console.log('✅ Usuario branch autenticado correctamente:', {
            branch_id: branch?.branch_id,
            branchname: branch?.branchname,
            email: branch?.email
        });
    }, [isAuthenticated, authType, isLoading, branch, navigate]);
    ...
    // LOADING CON INFORMACIÓN DE CONTEXTO
    if (isLoading) {
        return (
            <div className="flex flex-col items-center justify-center h-screen bg-gray-100">
                <div className="animate-spin rounded-full h-10 w-10 border-t-2 border-b-2 border-primary mb-4"></div>
                <p className="text-gray-600">Cargando dashboard de sucursal...</p>
            </div>
        );
    }
